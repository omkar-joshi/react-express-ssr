...
describe('routes#getRoute', () => {
  ...
  it('should return a nested route', () => {
    expect(getRoute('users.list', undefined, testRoutesConfig)).toEqual(
      '/users',
    );
  });

  it('should return a route with its parameters intact if no parameter was given', () => {
    expect(getRoute('users.view', undefined, testRoutesConfig)).toEqual(
      '/users/:id',
    );
  });

  it('should return a route with its parameters replaced', () => {
    expect(getRoute('users.view', { id: 12 }, testRoutesConfig)).toEqual(
      '/users/12',
    );
  });
